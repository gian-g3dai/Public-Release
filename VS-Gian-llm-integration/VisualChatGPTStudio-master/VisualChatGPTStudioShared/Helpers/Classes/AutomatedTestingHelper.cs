...
{
    internal class AutomatedTestingChatHelper : BaseChatHelper, IChatHelper
    {
        public void RefreshChat(OpenAIService llm)
        {
            ...
            ChatMaster.ChatType = Constants.AUTOMATEDTESTING_ID;
        }

        public override void AppendUserInput(OpenAIService llm, string prompt)
        {
            CommandText = TurboChatHelper.GetChatName(Constants.AUTOMATEDTESTING_NAME);
        ...
        public async Task<string> GetChatGPTResponseAsync(string prompt, AgentDTO agent, CancellationToken cancellationToken)
        {
            ...
            */
            ...
            else
            {
                ...
                await ChatGPT.GetResponseAsync(option, "", prompt, option.StopSequences.Split(','), HandlestreamContent, cancellationToken);
                OnRequestComplete(true);
                await System.Threading.Tasks.Task.Run(async () => await SaveChatAsync()).ConfigureAwait(false);
                return responseBuilder.ToString();
            }
        ...
        public override async void HandleSegmentsAsync(List<ChatTurboResponseSegment> segments)
        {
            foreach (var segment in segments)
            {
                // Determine the author based on whether the segment is code
                var author = AuthorEnum.ChatGPTCode;

                var item = AddChatItem(author, segment.Content, false, ChatItems.Count());
                item.Syntax = TextFormat.DetectCodeLanguage(segment.Content);
                ChatItems.Add(item);
                RefreshChatItem();
            ...
            foreach (var item in detailList)
            {
                ...
                else
                {
                    ...
                    if (aurthor == AuthorEnum.Me && ChatItems.Count == 0)
                    {
                        ...
                        chatItem.SecondTag = "Automated Testing";
                    }
